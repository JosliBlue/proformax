                ...
                @foreach ($data as $user)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                        ...
                        <td class="px-4 py-3 whitespace-nowrap">
                            <span
                                class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $user->user_status ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-200' : 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-200' }}">
                                <span class="iconify h-4 w-4 mr-1.5"
                                    data-icon="{{ $user->user_status ? 'heroicons:check-circle-20-solid' : 'heroicons:x-circle-20-solid' }}"></span>
                        ...
                        <td class="px-4 py-3 whitespace-nowrap">
                            <div class="flex items-center gap-2">
                                <!-- Botón Editar -->
                                <a href="{{ route('sellers.edit', $user->id) }}"
                                ...
                                <form id="deactivateForm-{{ $user->id }}"
                                    action="{{ route('sellers.soft_destroy', $user->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('PATCH')
                                    <button type="button" onclick="confirmDeactivate('{{ $user->id }}')"
                                        ...
                                        title="{{ $user->user_status ? 'Desactivar' : 'Activar' }}">
                                        <span class="iconify w-5 h-5"
                                            data-icon="{{ $user->user_status ? 'heroicons:eye-slash-20-solid' : 'heroicons:eye-20-solid' }}"></span>
                                    </button>
                                </form>

                                <!-- Botón Eliminar -->
                                <form id="deleteForm-{{ $user->id }}"
                                    action="{{ route('sellers.destroy', $user->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
    ...
    <script>
        ...
        function confirmDeactivate(userId) {
            showAlert({
                ...
                text: "¿Estás seguro de querer cambiar el estado de este vendedor?",
                icon: 'question',
                showCancelButton: true,
