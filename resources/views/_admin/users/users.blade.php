...
@section('content')
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-4">
        ...
        <div class="w-full sm:w-auto">
            <form action="{{ route('sellers') }}" method="GET" class="flex flex-col sm:flex-row items-center gap-2">
                {{-- Input de búsqueda --}}
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Buscar usuarios..."
            ...
            <tbody class="divide-y divide-gray-200">
                @foreach ($data as $user)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                            {{ ucfirst($user->user_rol->value) }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                            <span
                                class="{{ $user->user_status ? 'px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800' : 'px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800' }}">
                        ...
                        <td class="whitespace-nowrap text-sm text-gray-700">
                            <div class="flex space-x-2">
                                ...
                                <form id="deactivateForm-{{ $user->id }}"
                                    action="{{ route('sellers.soft_destroy', $user->id) }}" method="POST"
                                    class="inline">
                                    @csrf
                                    @method('PATCH')
                                    <button type="button" onclick="confirmDeactivate('{{ $user->id }}')"
                                        ...
                                        <span class="iconify w-6 h-6"
                                            data-icon="{{ $user->user_status ? 'mdi:eye-off' : 'mdi:eye' }}"></span>
                                    </button>
                                </form>
